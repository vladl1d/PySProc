...
def _table_fullname(table):
    return (table.schema if table.schema else 'dbo') + '.' + table.name
...
class ModelClass(Model):
    ...
    def __init__(self, table, association_tables, inflect_engine, detect_joined):
        ...
        self.name = self._tablename_to_classname(table.name, table.schema, inflect_engine)
        self.children = []
        ...
        for constraint in sorted(table.constraints, key=_get_constraint_sort_key):
            if isinstance(constraint, ForeignKeyConstraint):
                ...
                if (detect_joined and self.parent_name == 'Base' and
                    ...
                    if self.name == target_cls: continue #name<>target_cls
        ...
        for association_table in association_tables:
            ...
            target_cls = self._tablename_to_classname(
                ...
                fk_constraints[1].elements[0].column.table.schema,
                inflect_engine)
            relationship_ = ManyToManyRelationship(self.name, target_cls, association_table)
            self._add_attribute(relationship_.preferred_name, relationship_)

    @classmethod
    def _tablename_to_classname(cls, tablename, schema, inflect_engine):
        if not schema: schema = 'dbo'
        tablename = cls._convert_to_valid_identifier(tablename)
        camel_case_name =  tablename = (schema.upper() + '_' + tablename) if schema.lower() != 'dbo' else tablename
        #camel_case_name = '_'.join(part[:1].upper() + part[1:] for part in tablename.split('_'))
...
{models}"""
    ...
    def __init__(self, metadata, noindexes=False, noconstraints=False, nojoined=False,
        ...
        for table in metadata.sorted_tables:
            ...
            if noclasses or not table.primary_key or table.name in association_tables:
                ...
                if table.name in tables: continue #vvl: continue if exists
                tables[table.name] = model
            else:
                model = self.class_model(table, links[table.name], self.inflect_engine,
                                         not nojoined)
                if model.name in classes: continue #vvl: continue if exists
    ...
    def render_constraint(self, constraint):
        ...
        if isinstance(constraint, ForeignKey):
            remote_column = '{0}.{1}'.format(_table_fullname(constraint.column.table),
            ...
            remote_columns = ['{0}.{1}'.format(_table_fullname(fk.column.table), fk.column.name)
    ...
    def render_table(self, model):
        table_schema = model.table.schema.upper() if model.table.schema and model.table.schema != 'dbo' else ''
        table_name = (table_schema + '_' + model.table.name) if table_schema else model.table.name

        rendered = '{0} = Table(\n{2}{1!r}, metadata,\n'.format(table_name, model.table.name, self.indentation)
